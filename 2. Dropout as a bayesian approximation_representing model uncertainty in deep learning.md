...
- $\lbrace \mathbf x_1, \cdots, \mathbf x_N \},\{ \mathbf y_1, \cdots, \mathbf y_N \rbrace$: Training dataset.
...
- Define mean function and the covariance function of $\mathbf f(\x)$ as 
$$m(\mathbf x) = \E[\mathbf f(\x)]$$
$$k(\x, \x') = \E[(\mathbf f(\mathbf x) - m(\mathbf x)) (\mathbf f(\mathbf x') - m(\mathbf x'))^\top ]$$

- Write the Gaussian process as $$\mathbf f(\mathbf x) \sim \mathcal{GP}(m(\mathbf x), k(\mathbf x, \mathbf x')).$$


- In practice, we place a joint Gaussian distribution over all function values $$\mathbf F | \mathbf X \sim \mathcal N (\mathbf 0, \mathbf K(\mathbf X, \mathbf X))$$
