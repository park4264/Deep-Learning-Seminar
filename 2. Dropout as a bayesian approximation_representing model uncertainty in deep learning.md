...
We sample $T$ sets of vectors of realizations from the Bernoulli distribution $\lbrace \mathbf z^t_1, \cdots, \mathbf z^t_L \rbrace^T_{t=1}$ with $\mathbf z_i^t = [\mathbf z^t_{i, j}]^{K_i}_{j=1},$ giving $\lbrace \mathbf W^t_1, \cdots, \mathbf W^t_L \rbrace^T_{t=1}.$
