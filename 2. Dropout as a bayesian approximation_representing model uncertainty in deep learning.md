...
- Goal of a Bayesian neural network is to find a posterior distribution: $$p(\omega | \X, \Y) \propto p(\Y | \X, \omega) p(\omega)$$ where $\mathbf X$ and $\mathbf Y$ are the input and output training data and $\OMEGA$ is a set of parameters of our interest.
